...
import com.mrlou.message.Message_PropertiesAct;
import com.mrlou.message.Message_ShortHourseAct;
...
public class Fragment2 extends Fragment implements HttpListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		myView = inflater.inflate(R.layout.activity_myfragment, container,
				false);
	...
	private void loadurl() {
		...
		frsource = getArguments().getString("frsource");
						...
						new OnSheetItemClickListener() {
							...
							public void onClick(int which) {
								if (!Environment
									...
									Toast.makeText(getActivity(), "存储空间未就绪！",
	...
	final class DemoJavaScriptInterface {
		...
		@JavascriptInterface
		public void service() {
			startActivity(new Intent(getActivity(), Main_addServices.class));
		}

		// 去消息页面
		@JavascriptInterface
		public void goMSG() {
			startActivity(new Intent(getActivity(), Message_MainAct.class));
		}

		@JavascriptInterface
		public void msgServer(int num) {
			Log.e("传过来的数值", num + "");
			// 1是经纪服务，2是认证房源，3是新盘推荐，4是楼先生独家，5是联合办公室，6是增值服务
			switch (num) {
			case 1:
				startActivity(new Intent(getActivity(),
						Economic_ServiceAct.class));
				break;
			...
			case 3:
				startActivity(new Intent(getActivity(),
						Message_PropertiesAct.class));
				break;

			case 4:
				startActivity(new Intent(getActivity(),
						Message_ValuesHourseAct.class));
				break;

			case 5:
				startActivity(new Intent(getActivity(),
						Message_ShortHourseAct.class));
				break;
		...
		@JavascriptInterface
		public void yiLouBang() {
			startActivity(new Intent(getActivity(), Yi_Activity.class));
		}

		@JavascriptInterface
		public void goOther() {
			// 此方法在fragment2中使用即可，用于跳转到其他设置页面
			startActivity(new Intent(getActivity(), Other_SettingAct.class));
		}
		...
		public void href(final String prurl, final String param,
			...
			if (prurl.equals("ylb_index.html")
					|| prurl.equals("fangyuan_select_house_type.html")) {
				handle.post(new Runnable() {
					@Override
					public void run() {
						// baseurlString+prurl
						...
						try {
							...
							fangWenDatas.add(new FangWenData(baseurlString
									+ prurl, prams));
						} catch (UnsupportedEncodingException e) {
							e.printStackTrace();
						}
		...
		// @JavascriptInterface
		// public void href(final String prurl,final String param,final String
		// hide) {
		// Log.e("fragment2++++href+++++", "地址"+prurl+"参数"+param);
		//
		// handle.post(new Runnable() {
		...
		// int heightfirstview = firstwebView.getHeight();
		// Log.e("height+width",
		// "height"+heightfirstview+"width"+widthfirstview);
		// secondWebView.setVisibility(View.INVISIBLE);
		// LayoutParams praParams = new
		// LinearLayout.LayoutParams(widthfirstview,heightfirstview );
		// secondWebView.setLayoutParams(praParams);
		// secondWebView.loadUrl(baseurlString+prurl);
		// firsttwo="yes";
		// secondWebView.clearHistory();
		...
		// LinearLayout.LayoutParams(widthfirstview,heightfirstview );
		// secondWebView.setLayoutParams(praParams);
		// Log.e("++++++++++++++++++++++++++++执行222222222222222222",baseurlString+prurl);
		// secondWebView.loadUrl(baseurlString+prurl);
		// }
		// cur="href";
		// try {
		...
		public void logout() {
			handle.post(new Runnable() {
				@Override
				public void run() {
					if (TextUtils.isEmpty(getActivity().getSharedPreferences(
		...
		public void getuserinfo() {
			handle.post(new Runnable() {
				@Override
				public void run() {
					SharedPreferences userSharedPreferences = getActivity()
	...
	private final class ItemClickListener implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			case 5:// 复制
				...
				Toast.makeText(getActivity(), "复制成功", Toast.LENGTH_SHORT)
						.show();
				popupWindow.dismiss();
				break;
			case 6:// 新浪
	...
	private void FenxiangqqQz(String title, String content, String url) {
		QZoneShareContent qZoneShareContent = new QZoneShareContent(
				new UMImage(getActivity(), shareimg));
	...
	Handler requestHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			switch (msg.what) {
			case 0:
				Toast.makeText(getActivity(), errorString, Toast.LENGTH_SHORT)
						.show();
				break;
			case 1:
				Toast.makeText(getActivity(), errorString, Toast.LENGTH_SHORT)
				...
				break;
			case 2:
				userSharedPreferences = getActivity().getSharedPreferences(
	...
	public void uploadimgresult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == IMAGE) {
			if (data == null) {
				Toast.makeText(getActivity(), "从相册获取返回错误", Toast.LENGTH_SHORT)
						.show();
				return;
			}
			if (bdcut.equals("2")) {
		...
		if (requestCode == RESULT) {
			if (data == null) {
				Toast.makeText(getActivity(), "裁剪已取消", Toast.LENGTH_SHORT)
						.show();
				return;
			}
