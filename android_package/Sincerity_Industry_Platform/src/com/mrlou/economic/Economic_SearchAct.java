...
public class Economic_SearchAct extends Activity implements HttpListener {
	...
	private Economic_searchAdapter adapter, adapter2;
	private String str, str_02;
	private String search, search2, id, company;
	private Economic_SearchUtil search_util;
	private List<Economic_SearchUtil> list;
	...
	private void prepare() {
		...
		list = new ArrayList<Economic_SearchUtil>();
	...
	private void findView() {
		...
		adapter = new Economic_searchAdapter(Economic_SearchAct.this,
				str_search);
		...
		lv_search.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (search.equals("")) {
					write("0", position);
					Intent intent = new Intent(Economic_SearchAct.this,
		...
		adapter2 = new Economic_searchAdapter(Economic_SearchAct.this,
				str_history);
	...
	public void write(String flag, int potision) {
		if (flag.equals("0")) {
			list.add(new Economic_SearchUtil("2", str_name.get(potision),
					str_phone.get(potision), str_id.get(potision), "0"));
		} else {
			list.add(new Economic_SearchUtil(str_search.get(potision), "2", "2",
					"2", "1"));
		}
		for (int i = 0; i < str_names.size(); i++) {
			list.add(new Economic_SearchUtil(str_company.get(i), str_names.get(i), str_phones
...
//		}
	 ...
	 * @param list
	 */
	private void writeListIntoSDcard(List<Economic_SearchUtil> list) {
		if (new OutputUtil<Economic_SearchUtil>().writeListIntoSDcard(
				"mrlou.txt", list)) {
			
		} else {
			Toast.makeText(Economic_SearchAct.this, "写入SD卡失败", 2000).show();
		}
	}
	...
	private void readListFromSDcard() {
		List<Economic_SearchUtil> list = new InputUtil<Economic_SearchUtil>()
				.readListFromSdCard("mrlou.txt");
		if (list == null) {
			Toast.makeText(Economic_SearchAct.this, "SD卡读取失败", 2000).show();
		} else {
	...
	private Handler newHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 2:
				...
				break;
			case 99:
				Toast.makeText(Economic_SearchAct.this, "网络异常，请尝试继续搜索", Toast.LENGTH_SHORT)
						.show();
				break;
			}
		}
		...
		new Thread() {
			public void run() {
				try {
					...
					Economic_SearchAct.this.finish();
				} catch (Exception e) {
