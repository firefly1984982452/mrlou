...
var server = 'https://appapi.imrlou.com/';
...
function loginbytokenFailCallBack(msg){ // 登陆失败弹出提示
    // location.href = 'https://obj@openlogin';
...
(function(){
    ...
    app.href = function(pageName,params,hideBar){
        ...
        if(isIos){
	        if (pageName.indexOf('ylb_index.html')>-1 || pageName.indexOf('ylb_index')>-1) {
	    		yiLouBang();
	        }else{		        	
	            if(pstr){
	                pstr = '@' + pstr;
...
// 回退，跳转，地图，分享，隐藏底部栏，打开登陆页，关闭登陆页，图片上传
    ...
    app.goBuidMap = function(data){
        var title = encodeURIComponent(data.building_name);
        var address = encodeURIComponent(data.address);
        if(isIos){
            location.href = 'mrlou@navmap@' + title + '@' + data.lat + '@' + data.lon + '@' + address + '@' + data.building_id;
        }
        if(isAndroid){
            window.mrlou.navmap(title,data.lat,data.lon,address,data.building_id);
        }
    }
