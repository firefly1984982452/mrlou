        ...
        globalBind:function(){
            ...
            $(document).on('click','.send_code_btn',function(){
                ...
                ajax.getUserByMobilePhone(data,function(data) {
                        ...
                        closeLoading();
                        if(data.errorid == 0){
                            //window.location.href="https://obj@loginbytoken@"+data.token;
                            ...
                            // appDebug('ajax 获取token成功，将token发送给APP，token='+data.token,function(){
                                // window.location.href="https://obj@loginbytoken@"+data.token; // 将token发送给APP
                                // 定义一个在loginbyTOken成功之后的回调方法，每个页面自定义，可实现不同逻辑
                                // window.afterLoginByToken = function(){
                                    // window.location.href="https://obj@href@lxs_index.html"; // 页面跳转到楼先生首页
                                // }
                            // })
                        }
                    },function(data){
...
function UserInfoCallBack(session_id,user_id,type_id){
    ...
    // appDebug('收到session_id，点击确定关闭登陆页登陆,data:'+[session_id,user_id,type_id].join('，'),function(){
