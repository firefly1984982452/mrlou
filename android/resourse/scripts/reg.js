...
//						其它
			...
			globalBind:function(){
					...
					if(params){
						ajax.getUser(params,function(data){
							if(data.errorid == 0){
                                // app.href('lxs_index');
                                app.closeLogin();
								// window.location.href="http://obj@href@lxs_index.html";
							}
						})
					}
				})
				...
				$(document).on('focus','#building_name',function(){
					...
					var filter = buildingFilter = new Filter(document.getElementById('building_name'),{
						...
						noResult : function(){
							filter.dom.con.innerHTML = '<p style="padding:10px 0;text-align:center;font-size:14px;color:#444;">系统无法找到该楼盘，注册后客服会尽快与您联系。</p>';
						}
					})
				})
			...
			getSearchFormData:function(){
				...
				params.wechat_number = $("#add").find('#wechat_number').val();
				params.avatar = $('.img_del').attr('imgId');
				params.work_start = $('#add').find('#work_start').val();
				params.region_id = $('#add').find('#go_select_region').attr('vals');
				params.weixin_code = $(".img_weixin").attr("imgId");
				params.id_img = $(".img_id").attr("imgId");
		...
		var uploadCallBack = function(id,url){
			if(uploading == 'weixin_code'){
				setWeixinCover(id,url);
			}
			if(uploading == 'cover'){
				setCover(id,url);
			}
			if(uploading == 'id_cover'){
				setIDCover(id,url);
			}
			/*else{
				addImages(id,url);
		...
		function setWeixinCover(id,url){
            $('.weixin_code').remove();
			$('#wechat_code').hide();
