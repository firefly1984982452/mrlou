...
(function (global) {
     ...
     * @protected
     */
    iSlider.EVENTS = 'initialize slide slideStart slideEnd slideChange slideChanged slideRestore slideRestored reloadData destroy itemCreate'.split(' ');
    ...
    iSliderPrototype._renderWrapper = function () {
        ...
        for (var i = 0; i < 3; i++) {
            ...
            this.fire('itemCreate',li);
            this._opts.onItemCreated && this._opts.onItemCreated(li);
        }
    ...
    iSliderPrototype.destroy = function () {
        ...
        // Clear timer
        for(var p in this._LSN){
            clearTimerOnDestroy(this._LSN[p]);
        }
        function clearTimerOnDestroy(timer) {
